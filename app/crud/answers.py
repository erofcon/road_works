...
async def get_answers(task_id: int):
    answer_query = answers_model.answers.select().where(answers_model.answers.c.task_id == task_id)
    ...
    for answer in answers:
        ...
        answer_creator_query = users_model.users.select().where(users_model.users.c.id == answer.creator_id)
        ...
        one_answer.creator = users_schemas.UsersBase(**answer_creator)
...
async def get_one_answer(answer_id: int) -> answers_schemas.AnswerQuery:
    answer_query = answers_model.answers.select().where(answers_model.answers.c.id == answer_id)
