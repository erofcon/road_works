...
def create_access_token(
    ...
    return token_schemas.Token(access_token=access_encoded_jwt, refresh_token=refresh_encoded_jwt,
                               username=data['sub'], id=data['id'])


def create_refresh_token(token: str, username: str) -> token_schemas.Token | HTTPException:
    try:
        ...
        if datetime.utcfromtimestamp(payload.get('exp')) > datetime.now() and username == payload.get('sub'):
            return create_access_token(data={'sub': username, 'id': payload.get('id')})
